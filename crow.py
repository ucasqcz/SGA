...
def run_feature_processing_pipeline(features, d=128, whiten=True, copy=False, params=None):
    ...
    if type(features) == np.ndarray and len(features.shape) == 1:
    ...
    if params:
        ...
        # isomap = params['isomap']
        # features = isomap.transform(features)
        #lle = params['lle']
        #features = lle.transform(features)
    else:
        pca = PCA(n_components=d, whiten=whiten, copy=copy)
        features = pca.fit_transform(features)
        params = { 'pca': pca }
        # isomap = manifold.Isomap(n_neighbors=3,n_components=d)
        # features = isomap.fit_transform(features)
        # params = {'isomap': isomap}

        #lle = manifold.LocallyLinearEmbedding(n_neighbors=15,n_components=2,method = 'standard')
        #features = lle.fit_transform(features)
        #params = {'lle':lle}
